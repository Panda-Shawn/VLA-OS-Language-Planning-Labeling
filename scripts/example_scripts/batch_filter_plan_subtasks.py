...
if __name__ == "__main__":
    ...
    parser.add_argument("--task_plan_save_dir", type=str, default=None)
    ...
    if args.task_plan_save_dir is None:
        cot_dir = os.path.join(args.dataset_dir.replace("tfds_datasets", "planning_datasets"), data_name)
        os.makedirs(cot_dir, exist_ok=True)
        args.task_plan_save_dir = os.path.join(
    ...
    os.makedirs(args.task_plan_save_dir, exist_ok=True)
    ...
    if os.path.exists(args.task_metadata_path):
        ...
        for episode_id in primitives_dict.keys():
            # Get the task_id from episode_id to gather multiple episode_ids into a task_id
            task_id = episode_id.split("/")[-1][:9]
            # breakpoint()
    ...
    for episode_id in keys:
        ...
        save_paths.append(
            os.path.join(
                ...
                f"{episode_id.replace('/','_')}_{args.api_provider}.txt",
            )
        )
